...
def WebNovelEmbed(URL: str):
    ...
    try:
        if re.search(r"(www|m)\.webnovel\.com/book/", URL, re.IGNORECASE):
            ...
            DESCRIPTION = (
                WNinstance["SYNOPSIS"]
                if len(WNinstance["SYNOPSIS"]) < 350
                else f"{WNinstance['SYNOPSIS'][:345]} ..."
            )
            ...
            embed.add_field(
                name="Stats",
                value=f"""{WNinstance['VIEWS']} • {WNinstance['CHAPTERS']} • {WNinstance['RATINGS']}""",
                inline=False,
            )
            ...
            if WNinstance["RANKING"] is not None:
                embed.add_field(
                    name="Power Ranking", value=WNinstance["RANKING"], inline=True
                )

            embed.add_field(
                name="Tags", value=" • ".join(WNinstance["TAGS"]), inline=False
            )
