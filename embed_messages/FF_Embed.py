...
def FanFictionDotNetEmbed(URL: str):
    ...
    file = File(
        "./embed_messages/Logos/FanFictionDotNet.png", filename="FanFictionDotNet.png"
    )
    try:
        ...
        if FFReply["COVER_IMAGE"] is not None and FFReply["COVER_IMAGE"].startswith(
            "https://"
        ):
        ...
        if FFReply["CHARACTERS"] is not None:
            embed.add_field(
                name="Characters", value=str(FFReply["CHARACTERS"]), inline=True
            )
        ...
        embed.add_field(
            name="Status",
            value=f"*{FFReply['STATUS'].capitalize()}* • \
		Published on {FFReply['PUBLISHED'][:10]} • Updated on {FFReply['UPDATED'][:10]}",
            inline=False,
        )

        # Because why not?
        chapterstring = (
            f"{FFReply['CHAPTERS']} chapter"
            if int(FFReply["CHAPTERS"]) == 1
            else f"{FFReply['CHAPTERS']} chapters"
        )
        embed.add_field(
            name="Stats",
            value=f"{FFReply['RATING']} • {FFReply['WORDS']} words • {chapterstring}\
		 • {FFReply['LANGUAGE']}",
            inline=False,
        )
