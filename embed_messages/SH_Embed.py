...
def ScribbleHubEmbed(URL: str):
    ...
    try:
        if re.search(
            r"(^https://www\.scribblehub\.com/(series|read))/\d+", URL, re.IGNORECASE
        ):
            ...
            DESCRIPTION = (
                SHReply["SYNOPSIS"]
                if len(SHReply["SYNOPSIS"]) < 350
                else f"{SHReply['SYNOPSIS'][:345]} ..."
            )
            ...
            if SHReply["CONTENT_WARNING"] != "N/A":
                embed.add_field(
                    ...
                    value=SHReply["CONTENT_WARNING"],
                    inline=False,
                )

            embed.add_field(
                name="Stats",
                value=f"""{SHReply['VIEWS']} • {SHReply['FAVOURITES']} • {SHReply['CHAPTER_COUNT']} • {SHReply['READERS']}""",
                inline=False,
            )
            ...
            if SHReply["RELATED_SERIES"] is not None:
                embed.add_field(
                    name="Related Series", value=SHReply["RELATED_SERIES"], inline=False
                )
            ...
            r"(^https://www\.scribblehub\.com/profile)/\d+/(\w+)*", URL, re.IGNORECASE
        ):
            ...
            embed = Embed(
                title="ScribbleHub Profile", url=URL, description="", color=0xE09319
            )

            # Add author, thumbnail, fields, and footer to the embed
            embed.set_author(
                name=SHReply["AUTHOR_NAME"], url=URL, icon_url=SHReply["PROFILE_PHOTO"]
            )
            ...
            if SHReply["HOME_PAGE"] != "N/A":
                embed.add_field(
                    name="Home Page", value=SHReply["HOME_PAGE"], inline=False
                )

            embed.add_field(
                name="Last Active", value=SHReply["LAST_ACTIVE"], inline=True
            )
