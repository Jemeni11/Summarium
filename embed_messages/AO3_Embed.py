    ...
    if re.search(r"^https://archiveofourown\.org/works/\d+",url, re.IGNORECASE) \
      ...
      if AO3Reply['CHARACTERS'] != 'N/A' and AO3Reply['RELATIONSHIPS'] != 'N/A':
        if len(AO3Reply['CHARACTERS']) + len(AO3Reply['RELATIONSHIPS']) <= 10:
          embed.add_field(name="Characters + Relationships", value=f"{CHARACTERS[:-1]} • *{RELATIONSHIPS[:-1]}* ", inline=False) 
        else:
          embed.add_field(name="Characters + Relationships", value=f"{CHARACTERS} ... • *{RELATIONSHIPS}* ...", inline=False) 
      elif AO3Reply['CHARACTERS'] != 'N/A' and AO3Reply['RELATIONSHIPS'] == 'N/A':
        embed.add_field(name="Characters", value=', '.join(AO3Reply['CHARACTERS'][1:11]).strip(), inline=False)
      elif AO3Reply['RELATIONSHIPS'] != 'N/A' and AO3Reply['CHARACTERS'] == 'N/A':
        embed.add_field(name="Relationships", value=', '.join(AO3Reply['CHARACTERS'][1:11]).strip(), inline=False)
