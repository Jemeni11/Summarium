...
def FictionDotLiveEmbed(URL: str):
    ...
    try:
        ...
        DESCRIPTION = (
            FLReply["DESCRIPTION"]
            if len(FLReply["DESCRIPTION"]) < 500
            else f"{FLReply['DESCRIPTION'][:495]} ..."
        )

        embed = Embed(
            title=FLReply["TITLE"], url=URL, description=DESCRIPTION, color=0xFFFFFF
        )
        ...
        if len(FLReply["OTHER_AUTHORS"]) != 0:
            embed.add_field(
                name="Other Authors",
                value=" • ".join(FLReply["OTHER_AUTHORS"]),
                inline=False,
            )

        if FLReply["AUTHOR_NOTE"].strip() != "":
            embed.add_field(
                name="Author's Note", value=FLReply["AUTHOR_NOTE"], inline=False
            )

        if FLReply["STORY_STATUS"] is not None:
            embed.add_field(
                name="Story Status",
                value=f"{FLReply['STORY_STATUS']}".capitalize(),
                inline=True,
            )
