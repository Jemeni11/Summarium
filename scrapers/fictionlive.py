...
def fictiondotlive(url: str):
    ...
    if "a" not in response["u"][0].keys():
        AUTHOR_IMAGE = (
            "https://ddx5i92cqts4o.cloudfront.net/images/1e1nvq5tm_fllogo.png"
        )
    else:
    ...
    STORY_STATUS = (
        None if "storyStatus" not in response.keys() else response["storyStatus"]
    )
    CONTENT_RATING = (
        None if "contentRating" not in response.keys() else response["contentRating"]
    )
