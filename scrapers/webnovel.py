...
def webnovel(url: str):
    ...
    if (
        not soup.title.get_text(strip=True)
        == "404 - Webnovel - Your Fictional Stories Hub"
    ):
        ...
        mini_info_div = [
            i.get_text(strip=True) for i in info_div.h2.contents if i.get_text() != " "
        ]
        ...
        AUTHOR_PAGE = AUTHOR_SOUP.find_all(
            "div", {"class": "avatar-area mb32 pr", "data-id": "0"}
        )[0]
        AUTHOR_IMAGE = f"https:{AUTHOR_PAGE.img['src']}"

        RATINGS = info_div.find_all("p", class_="_score ell mb24 fs0")[0].get_text(
            strip=True
        )
        RATINGS_FINAL = ""
        for i in RATINGS:
        ...
        try:
            SYNOPSISVar = about_div.find(
                "div", class_="g_txt_over mb48 fs16 j_synopsis"
            )
            ...
            SYNOPSIS = about_div.find(
                "div", class_="g_txt_over mb48 fs16 j_synopsis"
            ).get_text(strip=True)

        TAGS_LIST = (
            about_div.find_all("div", class_="j_tagWrap")[0]
            .find("div", class_="m-tags")
            .contents
        )
        ...
        try:
            payload = {
                "_csrfToken": f"{csrf_token}",
                "bookId": f"{STORY_ID}",
                "bookType": 2,
                "novelType": 0,
            }
            random_variable = (
                "https://www.webnovel.com/go/pcm/powerStone/getRankInfoAjax"
            )
            # RANKING = about_div.find('div', class_="fl fs0 mr24 pt8 pb8 power-rank").strong.get_text(strip=True)
            RANKING = scraper.get(
                random_variable, headers=headers, params=payload
            ).json()["data"]["rank"]
            if RANKING == -1:
                RANKING = "N/A"
        except:
