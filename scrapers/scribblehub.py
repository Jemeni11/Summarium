...
class ScribbleHub:
    def __init__(self, url: str) -> None:
        self.url = url
        if re.search(
            r"(^https://www\.scribblehub\.com/read)/\d+", self.url, re.IGNORECASE
        ):
        ...
        self.webPage = self.scraper.get(self.url, headers=headers)
    ...
    def SHWork(self) -> typing.Union[dict, str]:
        ...
        COVER_IMAGE = (
            " "
            if not self.soup.find("div", class_="fic_image")
            else self.soup.find("div", class_="fic_image").contents[1]["src"]
        )

        # ++++++++++++++++++++++FIC STATS++++++++++++++++++++++
        STORY_TITLE = self.soup.find("div", class_="fic_title")["title"].strip()
        AUTHOR = self.soup.find("span", class_="auth_name_fic").text.strip()
        AUTHOR_PROFILE_LINK = self.soup.find("span", class_="auth_name_fic").parent[
            "href"
        ]
        ...
        GENRES = " • ".join(
            i.get_text() for i in self.soup.find_all("span", {"property": "genre"})
        )
        ...
        if "Fanfiction" in GENRES:
            ...
            FANDOM_SOUP_LIST = [
                i.get_text() for i in FANDOM_SOUP[1].contents if i.get_text()[-1] != " "
            ]
            FANDOM = " • ".join(FANDOM_SOUP_LIST)
        else:
            FANDOM = "N/A"

        # Some stories have a related series
        CLASS__WI_FIC_GENRE = self.soup.find_all("div", class_="wi_novel_title tags")
        RELATED_SERIES = None
        for i in CLASS__WI_FIC_GENRE:
            if i.getText(strip=True) == "Related Series":
                RELATED_SERIES_TEXT = i.next_sibling.get_text(strip=True)
                RELATED_SERIES_LINK = i.next_sibling.a["href"]
                RELATED_SERIES = f"[{RELATED_SERIES_TEXT}]({RELATED_SERIES_LINK})"

        CONTENT_WARNING_SOUP = self.soup.find("ul", class_="ul_rate_expand")
        CONTENT_WARNING = (
            "N/A"
            if not CONTENT_WARNING_SOUP
            else " • ".join(i.get_text() for i in CONTENT_WARNING_SOUP)
        )
    ...
    def SHProfile(self) -> typing.Union[dict, str]:
        ...
        STATS = [
            ...
            for i in HEADING.find("div", class_="p_pairstats").contents
            if i != "\n"
        ]
        JOINED = STATS[0][6:]
        FOLLOWERS = STATS[1][9:]
        ...
        PERSONAL_INFORMATION = self.soup.find(
            "div", class_="overview_title", text="Personal Information"
        )
        PERSONAL_TABLE = PERSONAL_INFORMATION.next_sibling.next_sibling.tbody
        PERSONAL_TABLE_STATS = [
            i.get_text(strip=True) for i in PERSONAL_TABLE if i != "\n"
        ]
        ...
        # ? Author Information
        AUTHOR_INFORMATION = self.soup.find(
            "div", class_="overview_title", text="Author Information"
        )
