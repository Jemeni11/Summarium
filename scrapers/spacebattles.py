...
class SpaceBattles:
	...
	def SBWork(self) -> dict:
		...
		BODY_CONTENT = BODY.find("div", class_="p-body-content").find("div", class_="p-body-pageContent")
		...
		TAGS = [i.get_text(strip=True) for i in DESCRIPTION_HEADER.find_all("li")[2].dl.dd.span.find_all("a")]

		THREADMARK_HEADER_STATS = BODY_CONTENT_BLOCK_ONE.find(
			"div", class_="threadmarkListingHeader-stats").find(
			"div", class_="pairJustifier").find_all("dl", class_="pairs")
